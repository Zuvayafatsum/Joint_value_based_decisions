...
Ind_Choice1_replication = (long_data_replication(:,19) + long_data_replication(:,20)) / 2;

Ind_Acc_replication = [Ind_Choice1_replication];

Dyad_Acc_replication = [long_data_replication(:,21)];

% Combine data into one matrix
data_replication = [Ind_Acc_replication, Dyad_Acc_replication];  % Now this is a 17x12 matrix (6 measurements per condition)
...
% Convert `long_data` to array
long_data_discovery = readtable('Discovery_Short_Acc.csv');
...
Ind_Acc_discovery = [Ind_Choice1_discovery];
Dyad_Acc_discovery = [long_data_discovery(:,3)];

% Combine data into one matrix
data_discovery = [Ind_Acc_discovery, Dyad_Acc_discovery];  % Now this is a 17x12 matrix (6 measurements per condition)
